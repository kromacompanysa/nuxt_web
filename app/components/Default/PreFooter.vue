...
<template>
  <section class="relative py-15 overflow-hidden bg-gray-50 text-gray-900 border-t border-gray-200">
    ...
    <div class="relative px-6 mx-auto max-w-7xl">
      <div class="grid grid-cols-1 gap-16 lg:grid-cols-3">
        ...
        <div class="flex flex-col self-start">
          <div class="lg:sticky lg:top-24">
            <div class="relative group">
              <div
                class="absolute -inset-1 bg-linear-to-r from-red-600/20 to-gray-200 rounded-3xl blur opacity-25 group-hover:opacity-40 transition duration-1000" />
            ...
            <div class="mt-10 space-y-4">
              <h2 class="text-4xl font-extrabold tracking-tight text-gray-900 lg:text-5xl">
                Kroma <span class="text-transparent bg-clip-text bg-linear-to-r from-red-600 to-gray-600">Hub</span>
              </h2>
        ...
        <div class="lg:col-span-2">
          <Swiper :modules="[Pagination, Grid]" :grid="{
            rows: 1,
            fill: 'row',
          }" :slides-per-view="1.1" :breakpoints="{
            640: { slidesPerView: 2, grid: { rows: 1, fill: 'row' } },
            1024: { slidesPerView: 3, grid: { rows: 1, fill: 'row' } },
          }" :space-between="20" :pagination="{
            clickable: true,
            ...
            <SwiperSlide
              ...
              <div :class="[
                ...
                <div class="flex gap-4 mb-6 relative z-10">
                  <a :href="getWhatsAppLink(nav)" target="_blank" rel="noopener noreferrer" aria-label="WhatsApp"
                    class="social-icon">
                    <img src="/icons/whatsapp.svg" class="w-5 h-5" alt="WhatsApp">
                  </a>
                  <a :href="nav.social?.instagramUrl || '#'" target="_blank" rel="noopener noreferrer"
                ...
                <div class="grid gap-8 relative z-10">
                  <div v-for="group in nav.items" :key="group.label" class="space-y-4">
                    <div class="flex items-center justify-between border-b border-gray-100 pb-2">
                      ...
                      <button v-if="group.items && group.items.length > 2"
                    ...
                    <ul class="space-y-3">
                      <li v-for="item in (group.items || []).slice(0, expandedGroups[group.label] ? undefined : 2)"
                        :key="item.label">
                        <a :href="item.href"
...
const getWhatsAppLink = (nav) => {
  const phone = nav.social?.whatsappPhone;
  const msg = nav.social?.whatsappMessage;
  return phone
